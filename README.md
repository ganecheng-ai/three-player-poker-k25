...
**v1.0.3** - 代码优化版本
- 移除 `DoudizhuRules` 不必要的实例化（使用静态方法直接调用）
- 修复 `sound.py` 循环导入问题，使用延迟导入 logger
- 代码架构优化，提升可维护性
...
### v1.0.3 (2026-03-13)
- 代码架构优化：移除 `ai_player.py` 中 `DoudizhuRules` 不必要的实例化
- 修复 `sound.py` 模块级别导入 logger 导致的潜在循环导入问题
- 使用延迟导入模式提升代码可维护性
- 优化代码质量，确保长期演进能力
